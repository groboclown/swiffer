...
SWFEvent.prototype._normalize = function () {
  ...
  if (this.attributes) {
    ...
    if (this.attributes.activityId) {
      ...
      // Timers, child workflows, and activities.
      // However, only Timers use this in the deciders.
  ...
  // JSON parse output if possible
  if (this.attributes && this.attributes.result) {
    var parsed;
    try {
      parsed = JSON.parse(this.attributes.result);
      this.attributes.result = parsed;
    } catch (err) {
      // Swallow.
    }
...
/**
 * Is this event a "Canceled" event?
 * @return {Boolean}
 */
SWFEvent.prototype.isCanceled = function () {
  return this.type.substr(-8) === 'Canceled';
};
...
SWFEvent.prototype.getActionType = function () {
  if (this.isFailure()) {
    return 'Failed';
  }
  if (this.isScheduled()) {
