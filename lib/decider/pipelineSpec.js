...
describe('Pipeline', function () {
  describe('Series Pipeline', function () {
    ...
    it('Execute logically generated tasks', function () {
      var pipeline = new Pipeline.Series([new Task({
        name: 'newTask',
        type: 'activity'
  ...
  describe('Dynamic tasks', function () {
    it('which chain to tasks which chain', function () {
      var pipeline = new Pipeline.Series([
        new Task({
          name: 'dynamicTimer',
          type: 'timer',
          delay: '10'
        }).onCompleted(function (lastEvent, events, eventlist) {
          return new Pipeline.Series([
            new Task({
      ...
      expect(next).toEqual([new actions.TimerAction('newTimer', 15)]);
    });
    
    it('which have multiple event listeners, but with the first event listener not ready yet.', function () {
      var pipeline = new Pipeline.Series([
        new Task({
          name: 'activity1',
          type: 'activity',
      ...
      var next = pipeline.getNextActions(new EventList([{
        ...
        eventId: "1",
        activityScheduledEventAttributes: {
      ...
      expect(next).toEqual([new actions.ScheduleAction('activity2', undefined, {
        version: '1.0',
