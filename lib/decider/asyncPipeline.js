 ...
 * @param {object} [p.timeouts] - timeout storage object
 * @param {string} [p.timeouts.lambdaStartToClose] - start to close timeout for
 *    the lambda.  Defaults to `300`.
 * @param {long} [p.timeouts.startToClose] - total time (in seconds) from when
 *    the spawned activity starts to when it is considered timed out.
 *    Defaults to `300`.
 * @param {long} [p.timeouts.scheduleToStart] - time (in seconds) allowed to
 ...
 * @param {boolean} [p.isStandaloneWorkflow] - is this run as a stand-alone
 *    workflow?
 * @param {function} [p.populateInput] - called when the lambda is ready to
 *    be scheduled.  Takes the arguments `(input_object, workflow_schedule_input)`
 */
var AsyncPipeline = function(p) {
  ...
  var input = p.input;
  p.timeouts = p.timeouts || {};
  var lambdaStartToCloseTimeout = p.timeouts.lambdaStartToClose || 300;
  var startToCloseTimeout = p.timeouts.startToClose || 300;
  var scheduleToStartTimeout = p.timeouts.scheduleToStart || 300;
  var scheduleToCloseTimeout = p.timeouts.scheduleToClose || 300; // Note: not currently used.
  var heartbeatTimeout = p.timeouts.heartbeat || 'NEVER'; // Note: not currently used.
  ...
  this._isStandaloneWorkflow = p.isStandaloneWorkflow || false;

  this._populateInput = p.populateInput;
  ...
  pipeline.Series.call(this, [
    ...
    new Task({
      ...
      input: input,
      timeouts: {
        startToClose: lambdaStartToCloseTimeout
      },
  ...
  this.onSignal(this._signalFailedName, {
    getNextActions: function (eventlist, fromEventId) {
      ...
      if (!isFinishedState(getState(stateName, eventlist))) {
        ...
        ret.push(new actions.FatalErrorAction('Activity failed', JSON.stringify(output)));
      }
...
AsyncPipeline.prototype.getNextActions = function (eventlist) {
  ...
  // the decider.
  if (!eventlist.workflowExecution || !eventlist.domain) {
    throw new Error('No workflowExecution or domain on eventlist');
  }
  ...
  if (isFailedState(state)) {
    // The pipeline failed.
    return this._finishActions(stateData, [
  ...
  if (isCompletedState(state)) {
    ...
    return this._finishActions(stateData, [], eventlist);
  }
  ...
  nextActions.forEach(function (a) {
    if (!!a && a instanceof actions.ScheduleLambdaAction) {
      ...
      if (self._populateInput) {
        var workflowEvent = _.findWhere(eventlist, {
          type: 'WorkflowExecutionStarted'
        });
        var workflowInput = (workflowEvent && workflowEvent.getOutput()) || {};
        self._populateInput(a._input, workflowEvent, eventlist);
      }
      a._input.async = {
        domain: eventlist.domain,
...
AsyncPipeline.prototype._finishActions = function (stateData, nextActions, eventlist) {
  var state = getStateFromData(stateData);
  ...
  // If we're completed, and there's no actions, then put in the
  // workflow exit note.  This should only be done if this is not
  // part of a larger workflow.

  if (!hasActionableActions(nextActions) && isCompletedState(state)) {
    if (this._isStandaloneWorkflow) {
      nextActions.push(new actions.ExitWorkflowAction(stateData.result || stateData.all));
    }
  }
  return nextActions;
};
...
function cancelTimer(timerName, eventlist) {
  // TODO look at using the new 'cancelTimer' task type instead.
  var ret = [];
  ...
  var canceledEvent = getTimerEventForType(startedEvent, 'canceled', eventlist);
  var failedEvent = getTimerEventForType(startedEvent, 'failure', eventlist);

  if (!!startedEvent && !completedEvent && !canceledEvent && !failedEvent) {
...
function getTimerEventForType(startedEvent, isType, eventlist) {
  if (!!startedEvent) {
    var timerId = startedEvent.attributes.timerId;
    for (var i = eventlist.length; --i >= 0;) {
      var evt = eventlist[i];
      var f = 'is' + isType[0].toUpperCase() + isType.substr(1);
      if (evt[f]()) {
