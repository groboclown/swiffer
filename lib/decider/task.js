...
/**
 * Registers a function to a specific event name.  The event name is the
 * AWS SWF event type name.  The function will be invoked with the
 * parsed Event object as the argument.  The 'on' event can return a
 * list of actions or tasks to be run.  Note that only end-of-life events
 * will trigger the 'on' event.
 */
Task.prototype.on = function on(eventName, func) {
...
Task.prototype._handleLastEvent = function (lastEvent, events, eventlist) {
  return (
    // First, check if the exact event type is registered.
    this._processEventHandlers(lastEvent.type, lastEvent, events, eventlist) ||
    // Then, check if the action category is registered.
    this._processEventHandlers(lastEvent.getActionType(), lastEvent, events, eventlist) ||
...
Task.prototype._getScheduleAction = function (eventlist) {
  ...
  case 'lambda':
    ...
    return new actions.ScheduleLambda(this.name, this.functionName, populateDynamicConfig(this.input, eventlist), options);
  case 'timer':
 ...
 */
Task.prototype._processEventHandlers = function (eventName, lastEvent, events, eventlist) {
  var handlerList = this._eventHandlers[eventName];
  if (handlerList && handlerList.length > 0) {
    var ret;
    for (var i = 0; i < handlerList.length; i++) {
      ret = handlerList[i].call(this, lastEvent, events, eventlist);
      if (ret) {
