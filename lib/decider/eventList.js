...
var EventList = function (data) {
  Array.call(this);
  this.workflowExecution = data.workflowExecution;
  ...
  this.forEach(function (evt) {
    ...
    if (evt.attributes) {
      relatedEvent = null;
      if ((evt.category === 'activity' || evt.category === 'lambdaFunction') && evt.attributes.scheduledEventId) {
...
EventList.prototype.filter = function (func) {
  var res = Array.prototype.filter.call(this, func);
  res.workflowExecution = this.workflowExecution;
  return res;
};
...
EventList.prototype.hasStartedOneTaskInList = function (list) {
  var started = this._getStartedTaskNames();
...
EventList.prototype.hasCompletedAllTasksInList = function (list) {
  var completed = this._getCompletedTaskNames();
  return _.difference(list, completed).length === 0;
};

EventList.prototype._getCompletedTasks = function () {
  return this._getTasksOfIsType('Completed');
};

EventList.prototype._getStartedTasks = function () {
  return this._getTasksOfIsType('Started');
};

EventList.prototype._getCompletedTaskNames = function () {
  return this._getTaskNamesOfType('Completed');
};

EventList.prototype._getStartedTaskNames = function () {
  return this._getTaskNamesOfType('Started');
};
...
EventList.prototype._getTasksOfIsType = function (type) {
...
EventList.prototype._getTaskNamesOfType = function (type) {
  if (!_.isFunction(SWFEvent.prototype['is' + type])) {
    throw new Error('Invalid type ' + type);
  }
  var result = [];
...
EventList.prototype.getMostRecentEventByName = function (name, type) {
  ...
  switch (type) {
  case 'started':
    list = this._getStartedTasks();
    break;
  case 'completed':
    list = this._getCompletedTasks();
    break;
  default:
    list = this;
  }
