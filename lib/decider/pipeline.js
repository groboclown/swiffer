...
Series.prototype._discoverNextActions = function (eventlist) {
  ...
  for (var i = 0; i < this._pipe.length; i++) {
    ...
    var nextActions = this._processActions(task.getNextActions(eventlist, afterEventId), eventlist);
    actions = actions.concat(nextActions);
    actions.lastEventId = nextActions.lastEventId || actions.lastEventId;
